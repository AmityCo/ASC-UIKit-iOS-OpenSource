//
//  AmityChatMessageSenderView.swift
//  AmityUIKit4
//
...
public struct AmityChatMessageSenderView: AmityElementView {

    public var pageId: PageId?
    public var componentId: ComponentId?
    public var id: ElementId { .senderMessageBubble }

    let message: MessageModel
    let messageAction: AmityMessageAction
    /// Live upload progress (0...1) for this message's media attachment while
    /// it's uploading; `nil` when not applicable.
                ...
                if message.type == .image {
                    ...
                    if message.syncState != .error,
                       let url = MessageLinkDetector.firstURL(in: message.text) {
                        MessageLinkPreviewView(url: url, isOwner: true)
                            .padding(.top, 12)
                    }
                }
            }
    ...
    var textContent: some View {
        ...
        if #available(iOS 15, *) {
            ...
            let bodyFont = AmityTextStyle.body(.clear).getUIFont()
