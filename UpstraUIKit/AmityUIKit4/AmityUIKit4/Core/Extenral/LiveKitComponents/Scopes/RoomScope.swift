...
public struct RoomScope<Content: View>: View {
    ...
    @Binding private var _enableMicrophone: Bool
    ...
    public init(room: Room? = nil,
                ...
                enableMicrophone: Binding<Bool> = .constant(false),
                ...
                @ViewBuilder _ content: @escaping () -> Content)
    {
        ...
        __enableMicrophone = enableMicrophone
    ...
    public var body: some View {
        _content()
            ...
            .onChange(of: _cameraPosition) { newPosition in
                Task {
                    await switchCameraPosition(to: newPosition)
                }
            }
            .onChange(of: _enableMicrophone) { isEnabled in
                Task {
                    try await _room.localParticipant.setMicrophone(enabled: isEnabled)
                }
            }
            .onAppear {
    ...
    private func switchCameraPosition(to position: AVCaptureDevice.Position) async {
        guard let cameraTrack = _room.localParticipant.firstCameraVideoTrack as? LocalVideoTrack else { return }
